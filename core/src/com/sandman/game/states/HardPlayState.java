 ...
 */
public class HardPlayState extends State {
    ...
    public HardPlayState(GameStateManager gam, float x, float y){
        super(gam);
        goten = new Goten(x, y);
        //Setting the camera
        cam.setToOrtho(false, Dragonball.WIDTH/ 2, Dragonball.HEIGHT / 2);
        bg = new Texture("buu.png");
        ...
        for(float i = 1; i <= TUBE_COUNT; i++){
            tubes.add(new Tube(i * (TUBE_SPACING + Tube.TUBE_WIDTH) + 450) );
            //if(i == 2){
    ...
    public void update(float dt) {
        ...
        for(int i = 0 ; i < tubes.size; i++){
            ...
            if(tube.collides(goten.getBounds())){
                gam.set(new ScoreScreen(gam, count,3));
            }
        }
