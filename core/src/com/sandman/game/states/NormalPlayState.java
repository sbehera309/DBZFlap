...
public class NormalPlayState extends State {
    ...
    private static final int TUBE_SPACING = 325;
    ...
    public NormalPlayState(GameStateManager gam, float x, float y){
        super(gam);
        goten = new Goten(x, y);
        //Setting the camera
        cam.setToOrtho(false, Dragonball.WIDTH/ 2, Dragonball.HEIGHT / 2);
        bg = new Texture("cell1.jpg");
        ...
        for(float i = 1; i <= TUBE_COUNT; i++){
            tubes.add(new Tube(i * (TUBE_SPACING + Tube.TUBE_WIDTH) + 350));
        }
    ...
    public void update(float dt) {
        ...
        for(int i = 0 ; i < tubes.size; i++){
            ...
            if(tube.collides(goten.getBounds())){
                gam.set(new ScoreScreen(gam, count, 2));
            }
        }
